...
var request = require('request'),
    ...
    configDir = path.datadir('npm-onupdate.info'),
...
function auth(callback) {
    ...
    if (accessToken) {
        ...
        coPrompt.confirm('\n  Have you used ' + chalk.bold('npm-onupdate.info') + ' before? (y/[n]) ')(function (nul, usedBefore) {
            if (usedBefore) {
...
function register(callback) {
    ...
    async.series([
        ...
        function promptForEmail(callback) {
            function getEmail() {
                coPrompt('  Email (not for spam): ')(function (nul, val) {
                    try {
                        check(val).isEmail();
                        email = val;
                        callback();
        ...
        function promptTOS(callback) {
            function getTOSAgree() {
                coPrompt.confirm('  Do you agree with:\n  - Privacy Policy (' + chalk.underline('http://npm-onupdate.info/policy') + ')\n  - Terms and Conditions (' + chalk.underline('http://npm-onupdate.info/terms') + ')\n  (y/[n]): ')(function (nul, val) {

                    if (!val) {
                        console.log(chalk.red('\n  You need to agree to the Terms of Service available at http://npm-onupdate.info/tos\n'));
                        return getTOSAgree();
                    }
