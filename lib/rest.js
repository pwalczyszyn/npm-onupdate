...
var request = require('request'),
    ...
    apiUrl = process.env.NODE_ENV == 'development' ? 'http://localhost:3000/api/v1' : 'https://onupdate-outofme.rhcloud.com/api/v1',
...
function auth(callback) {
    ...
    if (accessToken) {
        request.get({
            ...
            if (response.statusCode != 200) {
                if (response.statusCode == 401) { // Unauthorized
                    ...
                    console.log('\n  Locally stored access token is invalid!'.red);
        ...
        coPrompt.confirm('\n  Did you use ' + 'onUpdate.info'.bold + ' before? (y/[n]) ')(function (nul, usedBefore) {
            if (usedBefore) {
...
function authenticate(email, password, callback) {
    ...
    if (email && password) {
        ...
        console.log('\n  Please login with your credentials:'.grey);
        async.series([
...
function register(callback) {
    ...
    console.log('\n  Please provide new account credentials:'.grey);
    async.series([
        ...
        function promptTOS(callback) {
            function getTOSAgree() {
                coPrompt.confirm('  Do you agree with a Terms of Service available at the ' + 'http://onupdate.info/tos'.underline + '? (y/[n]): ')(function (nul, val) {
        ...
        function register(callback) {
            request.post({
                ...
                if (err) {
                    console.log('\n  Something went wrong: %s'.red, err.message);
                    process.exit(0);
                }
                ...
                if (response.statusCode !== 200) {
                    if (response.statusCode == 400) { // Validation error
                        ...
                        body.forEach(function (error) {
                            console.log('\n  * ' + ('[' + error.param + '] - ' + error.msg).red);
                        });
                    } else {
                        console.log('\n  Something went wrong:'.red, response.statusCode, ' - ', body);
                    }
                    console.log();
                    process.exit(0);
                }
